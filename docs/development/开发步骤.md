...
| 阶段 | 步骤 | 内容 |
|------|------|------|
| **阶段1: 项目初始化** | Step 1-5 | 项目搭建、基础配置、目录结构 |
| **阶段2: Supabase 后端** | Step 6-8 | 数据库设计、认证配置、RLS策略 |
| **阶段3: 基础布局** | Step 9-10 | 导航组件、布局结构 |
| **阶段4: 认证系统** | Step 11-13 | 登录注册、用户状态、路由守卫 |
| **阶段5: 核心功能** | Step 14-16 | 表单CRUD、题库组件、拖拽功能 |
| **阶段6: 表单功能** | Step 17-18 | 表单填写、数据统计 |
| **阶段7: 主题系统** | Step 19 | 8套主题切换 |
| **阶段8: 部署上线** | Step 20 | Vercel + Supabase 部署 |
...
**目标**: 初始化 Next.js 15 项目，配置 TypeScript 和 Tailwind CSS

**前置依赖**: Node.js 18+, pnpm

**执行提示词**:
```
请帮我创建一个 Next.js 15 项目，使用以下配置：
- 使用 pnpm 作为包管理器
- TypeScript: 是
- ESLint: 是
- Tailwind CSS: 是
...
4. 创建 src/lib/supabase/middleware.ts（认证中间件）
...
【自动验证】
完成后请自动执行以下验证：
1. 检查 src/lib/auth.ts 和 src/middleware.ts 是否存在
2. 验证中间件配置是否正确
