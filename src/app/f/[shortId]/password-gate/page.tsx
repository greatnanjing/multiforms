...
import { getFormByShortId } from '@/lib/api/forms'
...
export default function PasswordGatePage() {
    ...
    const loadForm = async () => {
      try {
        ...
        if (formData.access_type !== 'password') {
          // No password protection, redirect to form
          router.push(`/f/${shortId}`)
          return
        }
  ...
  // Focus input on mount and after form load
  useEffect(() => {
    if (!loading && form) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCorrect) {
        ...
        sessionStorage.setItem(`form_${shortId}_verified`, 'true')
        sessionStorage.setItem(`form_${shortId}_timestamp`, Date.now().toString())
  ...
  }, [password, form, shortId, attempts, maxAttempts, router])
